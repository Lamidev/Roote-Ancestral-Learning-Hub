...
//       ],
//       mode: 'payment',
//       success_url: successUrl || `${baseFrontendUrl}/payment-success?session_id={CHECKOUT_SESSION_ID}&email=${encodeURIComponent(studentEmail)}`,
//       cancel_url: cancelUrl || `${baseFrontendUrl}/payment-failed?email=${encodeURIComponent(studentEmail)}`,
...
//     const level = session.metadata.course_level;
...
//     let quizResult = await QuizResult.findOne({ studentEmail: studentEmail.toLowerCase().trim() });

//     if (!quizResult) {
//       console.log('⚠️ No quiz record found, creating from payment metadata. Level:', level);
...
//       console.log('✅ Created new student record from Stripe payment. Level:', level, 'CourseID:', validatedCourseId);
//     } else {
//       console.log('✅ Updating existing quiz record. Original Level:', quizResult.level, 'Original CourseID:', quizResult.courseId);
      
//       quizResult.paymentStatus = 'completed';
//       quizResult.transactionId = transactionId;
//       quizResult.paidAt = new Date();
      
//       if (!quizResult.courseId || quizResult.courseId === "643218529") {
//         console.log('🔄 Updating courseId to match level:', quizResult.level);
//         quizResult.courseId = getDefaultCourseId(quizResult.level);
//       }
      
//       await quizResult.save();
//       console.log('✅ Stripe payment confirmed. Final Level:', quizResult.level, 'Final CourseID:', quizResult.courseId);
//     }
...
//       console.log('✅ Welcome email sent. Level:', quizResult.level, 'CourseID:', quizResult.courseId);
//     } catch (emailError) {
...
//         currency: currency
//       });
//       console.log('✅ Admin notification sent. Level:', quizResult.level);
//     } catch (adminEmailError) {
//       console.error('❌ Failed to send admin notification:', adminEmailError);
//     }

//     console.log('🎉 Stripe payment processed successfully. Student:', studentEmail, 'Level:', quizResult.level, 'CourseID:', quizResult.courseId);
...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    // Use payment-return as the handler for both success and cancel
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'],
      line_items: [
        {
